$(document).ready(function () {
    ...
    $('button').click(function () {
        ...
        if (auteur.length < 101 && chapitre.length < 101 && citation.length < 65535 && picture.length < 65535) {
            //$.post permet d'envoyer derriere une URL mon nouveau commentaire
            $.post(urlCommentaire, { author: auteur, chapter: chapitre, episode: episode, content: citation, image: picture }).done(function () {
                console.log('nouvelle citation envoyée');
                // remise a zéro des inputs
                $('input').val('');
                $("#resultForm").text(`La citation a bien été enregistrée`);
            });
        } else { $("#resultForm").text(`Votre formulaire n'est pas bien remplis`) }
    });
